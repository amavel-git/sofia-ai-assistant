...
def update_wp_draft(workspace, draft):
    ...
    if not wordpress_id:
        print("No wordpress_id found. Use create_wordpress_draft.py first.")
        return None

    html_content = draft.get("html_content")
    if not html_content:
        print("No HTML content found. Run generate_ai_draft_content.py first.")
        return None

    endpoint = f"{workspace['domain']}/wp-json/wp/v2/{wp_config['content_endpoint']}/{wordpress_id}"
    ...
    if response.status_code not in [200, 201]:
        print("Error updating WordPress draft:")
        print(response.text)
        return None
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python app/update_wordpress_draft.py WORKSPACE_ID DRAFT_ID")
        print("Example: python app/update_wordpress_draft.py local.ao DRAFT-0001")
        return
